...
class MainWindow(QMainWindow):
    def on_resize(self):
        ...
        rect = lambda x, y, w, h: QtCore.QRect(int(x), int(y), int(w), int(h))

        self.car_image.setGeometry(rect(width * 0.275, height * 0.15, width * 0.3, height * 0.5))
        self.load_image.setGeometry(rect(width * 0.03, height * 0.2, width * 0.125, height * 0.05))
        self.line.setGeometry(rect(width * 0.179, height * 0.1, 20, height))
        self.line_2.setGeometry(rect(0, height * 0.06, width, 16))
        self.car_detection.setGeometry(rect(width * 0.625, height * 0.15, width * 0.3, height * 0.5))
        self.cropped_plat.setGeometry(rect(width * 0.25, height * 0.8, width * 0.2, height * 0.075))
        self.plate_characters.setGeometry(rect(width * 0.49, height * 0.8, width * 0.2, height * 0.075))
        self.start_detection.setGeometry(rect(width * 0.03, height * 0.26, width * 0.125, height * 0.05))
        self.load_video.setGeometry(rect(width * 0.03, height * 0.35, width * 0.125, height * 0.05))
        self.stop_video.setGeometry(rect(width * 0.03, height * 0.41, width * 0.125, height * 0.05))
        self.open_camera.setGeometry(rect(width * 0.03, height * 0.5, width * 0.125, height * 0.05))
        self.close_camera.setGeometry(rect(width * 0.03, height * 0.56, width * 0.125, height * 0.05))
        self.exit.setGeometry(rect(width * 0.03, height * 0.74, width * 0.125, height * 0.05))
        self.line_3.setGeometry(rect(width * 0.21, height * 0.69, width * 0.765, 20))
        self.plate_ocr.setGeometry(rect(width * 0.73, height * 0.8, width * 0.2, height * 0.075))
        self.trained_ocr.move(int(width * 0.29), int(height * 0.735))
        self.tesseract_ocr.move(int(width * 0.29 + 250), int(height * 0.735))
        ...
        self.label_4.setGeometry(rect((width - label_4_width + 1) * 0.5, height * 0.015, label_4_width + 1, label_4_height))
        ...
        self.label_2.setGeometry(rect(width * 0.21, height * 0.1, label_2_width, label_2_height))
        ...
        self.label_3.setGeometry(rect(width * 0.21, height * 0.73, label_3_width, label_3_height))
    ...
    def ensure_tesseract_ready(self):
        if self.tesseract_ready is True:
            return True

        if not self.reader.configure_tesseract():
            self.tesseract_ready = False
            QMessageBox.warning(
                self,
                "Tesseract indisponible",
                "Le mode Tesseract OCR n'est pas disponible.\n"
                "Installe Tesseract OCR puis relance l'application.\n"
                "Exemple chemin attendu: C:\\Program Files\\Tesseract-OCR\\tesseract.exe",
            )
            return False
    ...
    def ocr_switch(self, btn):
        if (btn.text() == self.trained_ocr.text()):
            ...
            if not self.ensure_tesseract_ready():
                self.ocrButtonGroup.blockSignals(True)
                self.trained_ocr.setChecked(True)
                self.ocrButtonGroup.blockSignals(False)
                self.ocr_mode = OCR_MODES.TRAINED
                return
    ...
    def apply_ocr(self):
        if (self.ocr_mode == OCR_MODES.TRAINED):
            ...
            try:
                ...
                QMessageBox.warning(self, "OCR Error", str(err))
                return
            if (len(plate_text) > 0):
